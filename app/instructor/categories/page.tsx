...
export default function CategoriesPage() {
  ...
  const handleOpenCreatePage = (category?: Category) => {
    if (category) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        console.error(data.error || 'Failed to save category');
      }
    } catch (error) {
  ...
  const performDelete = async (categoryId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/instructor/categories/${categoryId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <div className="w-full lg:ml-64 lg:w-[calc(100%-16rem)]">
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Categories</h1>
                </div>
              </div>
        ...
        >
          {!showCreatePage ? (
            <>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900">Manage Categories</h2>
                <button
          ...
          {!showCreatePage ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {categories.map((category) => (
                  <div key={category._id} className="bg-white rounded-lg shadow-md border border-gray-200 p-4 hover:shadow-lg transition-shadow">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="text-base font-bold text-gray-900">{category.name}</h3>
                      <div className="flex gap-1">
                        <button
                          onClick={() => handleOpenCreatePage(category)}
              ...
              {categories.length === 0 && (
                <div className="text-center py-12 bg-white rounded-lg shadow-md border border-gray-200">
                  ...
                  <p className="text-sm font-medium text-gray-600 mb-1">No categories yet</p>
                  <p className="text-xs text-gray-500 mb-4">Create your first category to organize your courses</p>
                  <button
                    onClick={() => handleOpenCreatePage()}
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6 max-w-2xl mx-auto">
